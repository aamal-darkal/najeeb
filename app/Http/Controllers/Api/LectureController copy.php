...
class LectureController extends Controller
{
    public function timeTable()
    {
        ...
        $data['friday'] = [];
        $data['saturday'] = [];
        $student = Student::where('user_id', Auth::id())->with('subjects.weekProgs')->first();
        $subjects  = $student->subjects;
        $subjectDetails = $student->subjects;
        $packagesNames = Package::select('id', 'name')->find($subjects->pluck('package_id')->unique());
        foreach ($subjects as $subject) {
            if (!$subject->week_progs) continue;
            foreach ($subject->week_progs as $weekProg) {   
                $day = $weekProg['day'];
